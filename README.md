...
## 步驟三之二：設定銀行轉帳確認信的觸發條件

銀行轉帳訂單不會像信用卡走綠界自動回調，是你人工核對匯款後手動把 Google Sheet 的「付款狀態」改成「已付款」。要讓這個動作自動寄確認信給客戶（同時也通知你自己），需要額外設定一個「可安裝觸發條件」：

1. Apps Script 編輯器左側點「觸發條件」（鬧鐘圖示）
2. 右下角「新增觸發條件」
3. 選擇要執行的函式：`onPaymentStatusEdit`
4. 事件來源：**來自試算表**
5. 事件類型：**已編輯**
6. 儲存，第一次會要求授權（因為要用 MailApp 寄信），照畫面同意即可

設定好之後，只要把某一列的「付款狀態」欄位手動改成「已付款」，且該列「付款方式」是「銀行轉帳」，就會自動寄一封確認信給客戶（Email 欄位那個地址），並寄一封通知信到你自己（`NOTIFY_EMAIL`）。信用卡訂單因為已經有綠界回調自動處理，不會被這個觸發條件重複通知。
...
## 提醒：Google Sheet 手動補欄位標題

因為 Sheet 已經有資料，程式不會自動改表頭。目前需要手動確認的欄位標題：
